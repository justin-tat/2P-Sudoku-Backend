...
gameRouter.post('/makeGame', (req, res) => {
  ...
  .then(toUsers => {
    let p1 = toUsers[0].rows[0];
    let p2 = toUsers[1].rows[0];
    let board = {boardState: toUsers[2], boardSolution: toUsers[3], holes: toUsers[4], game_id: toUsers[5], board_id: p1.board_id, opp_board_id: p2.board_id};
    res.send(board);
  })
  .catch(err => {
...
gameRouter.get('/getGame', (req, res) => {
  console.log("req.query from getGame: ", req.query);
  console.log("req.url from getGame: ", req.url);
  let newRating = 0;
...
gameRouter.put('/finishGame', (req, res) => {
  console.log("req.body.params from finish game: ", req.body.params);
  let args = req.body.params;
  let rating = 0;
